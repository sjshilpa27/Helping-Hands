...
//Update Code STart Here
    if (isset($_REQUEST["id"]) && isset($_REQUEST["status"]))
    {
        ...
        $qry = "SELECT * FROM entertainment WHERE cno=".$_REQUEST["id"];
//         echo $qry;
...
if (isset($_REQUEST["btnsave"])) {
    ...
    $source_file = $_FILES["upfile"]["tmp_name"];
        ...
        if (move_uploaded_file($source_file, $target_file)) {
            ...
            mysql_select_db("cc", $con);
            $qry = "INSERT INTO entertainment (ctype,cname,cdes,cpic,cgenre,cmrp,citem_for,cduration,ctime,uid)
                   VALUES ('" . $type . "','" . $name . "','" . $desc . "','" . $imagename ."','" . $genre . "','" . $mrp . "','" . $rd . "','" . $dur . "',now(),'" . $_SESSION["userid"] . "')";
  //          echo $qry;
...
if(isset($_REQUEST["btnupdate"]))
{
    ...
    $imagename = $_FILES["upfile"]["name"];   
        $con=  mysql_connect("localhost","root","") or die("Sorry!!!Cannot Connect");
        mysql_select_db("cc",$con);
       $qry="UPDATE entertainment SET ctype='" . $type . "',cname='" . $name . "',cgenre='" . $genre . "',
               cmrp='" . $mrp . "',cdes='" . $desc . "''
...
<!--        <iframe width="100%" height="200px" src="head.php"  frameborder="0" style="border:0" allowfullscreen ></iframe>-->
        ...
        <div id="window">
         ...
         <form name="entertainment" action="entertainment.php" method="POST" enctype="multipart/form-data">
            <table border="0">
                  <tr>
                    <td colspan="2">
                        <h2>ENTERTAINING STUFFS</h2>
                    </td>
                </tr>         
 ...
 <td><select name="txtgenre" class="it">
   ...
   <?php
                    ...
                    if (isset($_REQUEST["id"]) && isset($_REQUEST["status"]))
                    {
                 echo'<input type="submit" name="btnupdate" value="Update" class="it">';
                  
                    }
                    else
                    {
                                  
                   echo" <input type='submit' name='btnsave' value='Save'  class='it'>";
                    }
                 ?>
